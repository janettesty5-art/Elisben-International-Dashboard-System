...
class DownloadResultPDF(LoginRequiredMixin, WeasyTemplateResponseMixin, DetailView):
    """Admin PDF Download"""
    model = StudentResult
    template_name = 'result/print_result.html'
    pk_url_kwarg = 'result_id'
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        result = self.get_object()
        context['subject_results'] = SubjectResult.objects.filter(
            student=result.student,
            term=result.term,
            academic_year=result.academic_year
        ).order_by('subject_name')
...
class DownloadStudentResultPDF(WeasyTemplateResponseMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['result'] = published.result
        context['published'] = published
        context['subject_results'] = SubjectResult.objects.filter(
            student=published.result.student,
            term=published.result.term,
            academic_year=published.result.academic_year
        ).order_by('subject_name')
        return context